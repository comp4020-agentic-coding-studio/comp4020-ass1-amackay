...
import { parsePalette, template } from "./index";
...
describe("the shipped palettes", () => {
  ...
  it("carries the design-study set: variable chips, wi, ax-1", () => {
    ...
    expect(p.templates.map((t) => t.label)).toEqual(["wi", "ax-1"]);
  });
  ...
  it("carries the prototype set, ax-mp included", () => {
    ...
    expect(p.templates.map((t) => t.label).sort()).toEqual([
      "ax-1", "ax-2", "ax-3", "ax-mp", "wi", "wn",
    ]);
    // ax-mp is the only template with locks, which is what makes it the one that
    // turns a pile of wffs into a derivation.
    expect(template(p, "ax-mp").locks).toHaveLength(2);
    for (const label of ["wn", "wi", "ax-1", "ax-2", "ax-3"]) {
      expect(template(p, label).locks).toEqual([]);
    }
  });
});
...
describe("assertPalette rejects", () => {
  ...
  it("a missing top-level key", () => {
    expect(parse((p) => delete (p as Partial<typeof p>).templates)).toThrow(
      /templates must be an array/,
    );
  });

  it("a typecode that isn't one", () => {
    expect(parse((p) => (p.templates[0].sockets[0].typecode = "wof"))).toThrow(
      /sockets\[0\].typecode must be one of/,
    );
  });

  it("an expression that doesn't start with a typecode", () => {
    expect(parse((p) => (p.templates[0].conclusion = ["-.", "ph"]))).toThrow(
  ...
  it("an empty expression", () => {
    ...
    expect(parse((p) => (p.templates[0].conclusion = []))).toThrow(
      /conclusion must have at least its typecode/,
    );
    expect(parse((p) => (p.templates[0].conclusion = ["wff"]))).not.toThrow();
  });

  it("a token that isn't a string", () => {
    expect(parse((p) => (p.templates[0].conclusion[2] = 7 as unknown as string))).toThrow(
  ...
  it("a token carrying whitespace", () => {
    ...
    expect(parse((p) => (p.templates[0].conclusion[2] = "ph ps"))).toThrow(
      /conclusion\[2\] must not contain whitespace/,
    );
  });

  it("a label used twice", () => {
    expect(parse((p) => p.templates.push({ ...p.templates[0] }))).toThrow(/repeats the label wn/);
  });

  it("a variable used with no socket to fill it", () => {
    // The dropped-socket case: ax-mp losing the ps it inherits from its
    // enclosing scope would look exactly like this.
    expect(parse((p) => (p.templates[0].sockets = []))).toThrow(
      /uses ph but declares no socket for it/,
    );
  });
});
